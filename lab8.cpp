    ...
    if (argc ==3) {
        ...
        for (int i=0; i<81;i++) {
            for (int j=0;j<5;j++) {
                cout << testData[i*5+j] << "  ";
                if (j==4) {
                    sbsi.h = testData[i*5+0];
                    sbsi.wc = testData[i*5+1];
                    sbsi.bsa = testData[i*5+2];
                    sbsi.vtc = testData[i*5+3];
                    Flt functVal = bodyShapeIndex(&sbsi);
                    cout << functVal << " ";
                    if (functVal < testData[i*5+4] - tolerance || 
                            functVal > testData[i*5+4] + tolerance) {
        ...
        cout << "bad element cout: " << bElement << endl;
        cout << "unit test complete." << endl;
        return 0;
    }
    if(argc >3) {
        cout << "too many cmdline arguements" << endl;
    }
    //
